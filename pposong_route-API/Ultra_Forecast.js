...
async function get_Ultra_Forecast_Data(input_date, input_time, input_x, input_y) {
    ...
    if (input_time % 100 < 45) {
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {   // 오류 발생시 try catch로 초단기예보API 최대 3번 재호출
            try {
    ...
    // 13:50 검색 --> 13(초단기실황) + 14, 15, 16, 17, 18, 19(초단기예보)
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) { // 오류 발생시 try catch로 초단기실황API 최대 3번 재호출
            try {
...
//             return ultra_forecast_datas;
//         } catch (error) {
//             console.error(attempt + `Attempt[${attempt}] failed`, error);
//             if (attempt >= maxAttempts)
